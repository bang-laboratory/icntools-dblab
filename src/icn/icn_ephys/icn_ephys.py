...
import icn.icn_stats as stats
import icn.icn_tb as tb
import mne
import numpy as np
import pandas as pd
import pyedflib
...
def normalize_spectrum(mpow, f):
    ...
    spow = mpow.copy()
    fnorm = np.append(
        np.arange(f.searchsorted(5), f.searchsorted(45) + 1),
...
def rox_burst_duration(bdata, bthresh, sfreq, min_length):
    ...
    if not any(i):
        bdur, n, bamp, peaktime, btime, ibidur, ibiamp = (
            np.nan,
            0,
            np.nan,
            np.nan,
    ...
    istart, istop, bdur, ibidur = (
        i,
        s,
        (s - i) / sfreq * 1000,
        (i[1:] - s[:-1] - 1) / sfreq * 1000,
    )
    bamp, ibiamp, btime, peaktime = (
        np.array([]),
        np.array([]),
        np.array([]),
        np.array([]),
    ...
    for n, dur in enumerate(bdur):
        ...
        peaktime = np.append(
            ...
            - 1
            + np.where(
                bdata[istart[n] : istop[n] + 1]
                == np.amax(bdata[istart[n] : istop[n] + 1], axis=None)
            ),
        )
...
def mne_write_edf(mne_raw, fname, picks=None, tmin=0, tmax=None, overwrite=False):
    ...
    mne.export.export_raw(
        fname, mne_raw, overwrite=overwrite, fmt="edf", add_ch_type=True
    )
...
def mne_write_bids(filename, subject, bids_folder="/bids", task="rest"):
    ...
    write_raw_bids(
        raw, BIDSPath(subject=subject, task=task), bids_folder, overwrite=True
    )
...
def mne_annotations_replace_dc(raw):
    raw.annotations.duration[raw.annotations.description == "DC Correction/"] = 6
    raw.annotations.onset[raw.annotations.description == "DC Correction/"] = (
        raw.annotations.onset[raw.annotations.description == "DC Correction/"] - 2
    )
...
def mne_epoch(
    raw, tmax=3.0, tmin=None, events=None, event_id="rest", reject_by_annotation=True
):
    if events is None:
...
def mne_filter(raw, freqs=None):
    ...
    if freqs is None:
        raw.notch_filter(
            np.arange(50, int(raw.info["sfreq"]) + 1, 50),
            method="spectrum_fit",
            n_jobs=tb.n_jobs(),
        )
...
def mne_eeg_remove_eyeblinks(raw, channels=None, overwrite=False):
    ...
    if not pathlib.Path.is_file(pathlib.Path(icafile)) or overwrite:
        filt_raw = raw.copy()
        filt_raw.load_data().filter(l_freq=1.0, h_freq=None, n_jobs=tb.n_jobs())
...
def mne_eeg_inspect_data_quality(raw, overwrite=False):
    ...
    for n, a in enumerate(bad):
        good_samples[
            int(raw.time_as_index(a["onset"])) : int(
                raw.time_as_index(a["onset"] + a["duration"])
            )
        ] = 0
    dataquality.update(
        {"good_seconds": np.round(np.sum(good_samples) / raw.info["sfreq"], 2)}
    )
    dataquality.update(
        {"pct_good_samples": np.round(100 * np.sum(good_samples) / raw.n_times, 2)}
    )
...
def mne_mark_visual_artifacts(
    raw, overwrite=False, mark_flat=True, remove_eye_blinks=True, overwrite_ransac=False
):
    ...
    if not pathlib.Path.is_file(pathlib.Path(filename)) or overwrite:
        ...
        chans = pd.read_csv(
            tb.fileparts(raw.filenames[0], "_channels.tsv", -4), delimiter="\t"
        )
...
def mne_crop_artifacts(raw):
    ...
    for n, a in enumerate(bad, 0):
        ...
        if 0 < start < stop < raw._last_time:
            cropped_raw = mne.concatenate_raws(
                [cropped_raw, raw.copy().crop(start, stop)]
            )
            print((start, stop))
    return cropped_raw
...
def mne_lcmv_to_raw(lcmv):
    ...
    info = mne.create_info(ch_names=ch_names, ch_types=ch_types, sfreq=lcmv.sfreq)
...
def mne_tf_wavelet(
    inst, freqs=np.arange(3, 100), n_freq=20, n_cycles=7.0, zero_mean=True
):
    wav = mne.time_frequency.tfr_morlet(
        inst,
        freqs,
        n_cycles=n_cycles,
        zero_mean=zero_mean,
        use_fft=False,
        return_itc=False,
        decim=int(inst.info["sfreq"] / n_freq),
        n_jobs=1,
        picks=None,
        average=False,
        output="power",
...
def mne_burst_analysis(
    ...
    for fband, frange in freqranges.items():
        ...
        if method == "hilbert":
            df = (
                raw.copy()
                .filter(l_freq=frange[0], h_freq=frange[1], n_jobs=tb.n_jobs())
            ...
            for ch in raw.ch_names:
                df.loc[:, ch] = (
                    df[ch]
                    .rolling(axis=0, window=int(smoothing / 1000 * raw.info["sfreq"]))
                    .mean()
                    .astype("float")
            ...
            df = pd.DataFrame(
                np.squeeze(wav.data.mean(axis=2)).transpose(), columns=raw.ch_names
            )
...
def mne_plot_raw(raw, block=False, outfile=None, show=True):
    ...
    raw_fig = raw.plot(
        ...
        filtorder=0,
        show_options=True,
        scalings={"eeg": 25e-6, "misc": 1},
        block=block,
        title=raw.filenames[0],
        remove_dc=True,
        show=show,
    )
    if outfile:
...
def mne_plot_psd_topo(raw, block=False, outfile=None, show=True):
    raw_fig = raw.plot_psd_topo(
        fmin=0.1, fmax=40, n_fft=int(3 * raw.info["sfreq"]), block=block, show=show
    )
    if outfile:
...
def mne_plot_ica(raw, n_components=15):
    ...
    pow, f = mne.time_frequency.psd_welch(
        ica_raw, n_fft=int(3 * ica_raw.info["sfreq"]), fmin=2, fmax=40
    )
    mpow = pow.mean(axis=0)
