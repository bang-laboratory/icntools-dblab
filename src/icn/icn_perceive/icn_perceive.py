    ...
    if "LfpFrequencySnapshotEvents" in data["DiagnosticData"]:
        ...
        for a in eventlist:
            if "LfpFrequencySnapshotEvents" in a.keys():
                ...
                n += 1
                outname = (
                    reformat_DateTime(a["DateTime"])
                    + "_"
                    + a["EventName"].replace(" ", "_")
                    + "_ID"
                    + str(a["EventID"])
                )
                ...
                spectra.loc[:, outname + "_" + ch_l] = lfpl["FFTBinData"]
                pfig.savefig(
                    str(
                        pathlib.Path(
                            fpath[0],
        ...
        spectra.to_csv(
            pathlib.Path(fpath[0], "figures", outname + "_snapshot_psd.tsv"), sep="\t"
        )


def import_LfpMontageTimeDomain(filename) -> Optional[mne.io.Raw]:
    data = read_file(filename)
    if "LfpMontageTimeDomain" in data.keys():
        ...
        meas_date = dateutil.parser.parse(
            data["LfpMontageTimeDomain"][0]["FirstPacketDateTime"]
            .replace("T", " ")
            .replace("0Z", "")
        )
        raw = ephys.mne_import_raw(
        ...
        print("NOTE: LfpMontageTimeDomain not found")
        return None
...
def import_BrainSenseTimeDomain(filename):
    ...
    if "BrainSenseTimeDomain" in data.keys():
        ...
        #     rsp = (
        #         str(rp["PulseWidthInMicroSecond"])
        #         + "_us_"
        #         + str(rp["RateInHertz"])
        #         + "_Hz"
        #     )
        #     lsp = (
        #         str(lp["PulseWidthInMicroSecond"])
        #         + "_us_"
        #         + str(lp["RateInHertz"])
        #         + "_Hz"
        #     )
        ...
        meas_date = dateutil.parser.parse(  # TODO (malte): isoparser?
            data["BrainSenseTimeDomain"][0]["FirstPacketDateTime"]
            .replace("T", " ")
            .replace("0Z", "")
        )
        raw = ephys.mne_import_raw(
        ...
        print("NOTE: BrainSensTimeDomain not found")
        return None
    ...
    if "IndefiniteStreaming" in data.keys():
        ...
        for ch in uchs:
            ...
            if isinstance(cdata, type([])):
                ...
                raw_data.loc[:, ch] = cdata.transpose()
        meas_date = dateutil.parser.parse(
            data["IndefiniteStreaming"][0]["FirstPacketDateTime"]
            .replace("T", " ")
            .replace("0Z", "")
        )
        raw = ephys.mne_import_raw(
        ...
        print("NOTE: IndefiniteStreaming not found")
        return None
...
def plot_LFPMontage_spectra(filename):
    ...
    if "LFPMontage" in data.keys():
        ...
        spectra = pd.DataFrame(
            columns=columns, index=data.LFPMontage[0]["LFPFrequency"]
        )
        ...
        for a in ir:
            ...
            if not artefact:
                plt.scatter(
                    lfp["PeakFrequencyInHertz"], lfp["PeakMagnitudeInMicroVolt"]
                )
        ...
        for a in il:
            ...
            if "PeakFrequencyInHertz" in lfp:
                plt.scatter(
                    lfp["PeakFrequencyInHertz"], lfp["PeakMagnitudeInMicroVolt"]
                )
            spectra[chans[a]] = lfp["LFPMagnitude"]
        ...
        if not os.path.isdir(pathlib.Path(fpath[0], "figures")):
            os.makedirs(pathlib.Path(fpath[0], "figures"))
        pfig.savefig(
            str(pathlib.Path(fpath[0], "figures", outname + "_LFPMontage_raw.png")),
            dpi=300,
        )
        spectra.to_csv(
            pathlib.Path(fpath[0], "figures", outname + "_LFPMontage_raw.tsv"), sep="\t"
        )
    else:
...
def plot_BrainSenseLfp(filename):
    ...
    if "BrainSenseLfp" in data.keys():
        ...
        if not os.path.isdir(pathlib.Path(fpath[0], "figures")):
            os.makedirs(pathlib.Path(fpath[0], "figures"))
        plt.gcf().savefig(
            str(pathlib.Path(fpath[0], "figures", outname + "_BrainSenseLfp.png")),
            dpi=300,
        )
        df.to_csv(
            pathlib.Path(fpath[0], "figures", outname + "_BrainSenseLfp.tsv"), sep="\t"
        )
    else:
...
def plot_wavelet_spectra(filename, typefield="all"):
    ...
    for f in flist:
        ...
        if raw is not None:
            ...
            for a in i:
                plt.plot(wav.freqs, rpow[a, :].transpose(), linewidth=0.5)
            ...
            for a in i:
                plt.plot(wav.freqs, rpow[a, :], linewidth=0.5)
                plt.title(a)
            plt.legend([wav.ch_names[ir] for ir in i])
            plt.plot(wav.freqs, rpow[i, :].mean(axis=0), linewidth=2, color="k")
            plt.ylim((0, 10))
            ...
            plt.title("Left hemisphere")
            outname = (
                data["SessionDate"][:-1].replace("-", "").replace(":", "") + "_" + f
            )
            plt.suptitle(outname)
            if not os.path.isdir(pathlib.Path(fpath[0], "figures")):
                os.makedirs(pathlib.Path(fpath[0], "figures"))
            pfig.savefig(
                str(pathlib.Path(fpath[0], "figures", outname + "_rpow.png")), dpi=300
            )
            spectra = pd.DataFrame(
                np.transpose(rpow), index=wav.freqs, columns=wav.ch_names
            )
            spectra.to_csv(
                pathlib.Path(fpath[0], "figures", outname + "_rpow.tsv"), sep="\t"
            )
            spectra = pd.DataFrame(
                np.transpose(spow), index=wav.freqs, columns=wav.ch_names
            )
            spectra.to_csv(
                pathlib.Path(fpath[0], "figures", outname + "_spow.tsv"), sep="\t"
            )
            spectra = pd.DataFrame(
                np.transpose(mpow), index=wav.freqs, columns=wav.ch_names
            )
            spectra.to_csv(
                pathlib.Path(fpath[0], "figures", outname + "_mpow.tsv"), sep="\t"
            )
...
def LfpMontageTimeDomain_to_bids(
    ...
    if raw is not None:
        ...
        print("hi2")
        ephys.write_raw_bids(raw, bpath, overwrite=True)
        print("hi3")
        # shutil.copyfile(
        #     filename,
        #     pathlib.Path(
        #         bids_folder, bpath, "eeg", "sourcedata", basename.basename + ext
        #     ),
        # )
        plot_wavelet_spectra(filename, typefield=task)
        plot_LFPMontage_spectra(filename)
        os.remove("tmp.edf")
        # shutil.move(
        #     pathlib.Path(
        #         bids_folder, bpath, "eeg", "sourcedata", basename.basename + ext
        #     ),
        #     pathlib.Path(
        #         bids_folder, bpath, "eeg", "sourcedata", sourcename.basename + ext
        #     ),
        # )
    return raw


def BrainSenseTimeDomain_to_bids(
    filename, bpath, task="BrainSenseTimeDomain", events=None, event_id=None
):
    # data = read_file(filename)
    # opath, fname, ext = tb.fileparts(filename)
    # if subject.find("-") > 0:
    ...
    bpath.update(task=task)
    sourcedata = bpath.directory / "sourcedata"
    figure_path = bpath.directory / ".." / "figures"
    tb.mkdir(figure_path)
    # print(bpath, sourcedata, figure_path)
    raw = import_BrainSenseTimeDomain(filename)
    if raw is not None:
        rfig = raw.plot(color="k")
        rfig.savefig(figure_path / (bpath.basename + "_figures.png"))
        if os.path.exists("tmp.edf"):
            os.remove("tmp.edf")
        ephys.mne_write_edf(raw, "tmp.edf")
        raw = mne.io.read_raw_edf("tmp.edf", infer_types=True)
        write_raw_bids(raw, bpath, overwrite=True, events=events, event_id=event_id)
        # shutil.copyfile(
        #     filename,
        #     pathlib.Path(bids_folder, bpath, "eeg", "sourcedata", basename + ext),
        # )
        plot_wavelet_spectra(filename, typefield=task)
        plot_BrainSenseLfp(filename)
        os.remove("tmp.edf")
        # shutil.move(
        #     pathlib.Path(bids_folder, bpath, "eeg", "sourcedata", basename + ext),
